...
class AnchorTargetCreator(object):
    ...
    def __call__(self, gt_bboxes, anchors, img_size):
        ...
        inside_idx = torch.where(
            ...
            (anchors[:, 2] <= img_h) &
            (anchors[:, 3] <= img_w)
        )[0]  # [0] is for eliminating the influences from the batch
        ...
        gt_locs = bbox2loc(anchors, gt_bboxes[anchor_argmax_ious])
...
class ProposalTargetCreator(object):
    ...
    def __call__(
        ...
        n_bboxes = gt_bboxes.shape[0]
        n_rois = rois.shape[0]

        ious = bbox_iou(rois, gt_bboxes)
        ...
        gt_roi_labels = gt_labels[roi_argmax_ious] + 1
        gt_roi_labels = gt_roi_labels[keep_idx]
        gt_roi_labels[len(pos_idx):] = 0  # negative labels --> 0
        roi_samples = rois[keep_idx]

        # Compute offsets and scales to match sampled rois to the GTs.
        gt_roi_locs = bbox2loc(roi_samples, gt_bboxes[roi_argmax_ious[keep_idx]])
        gt_roi_locs = ((gt_roi_locs - torch.Tensor(loc_normalize_mean).type(torch.float32))
                       / torch.Tensor(loc_normalize_std).type(torch.float32))
